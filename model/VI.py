...
class MODEL:
    ...
    def fit(self, nlp_tol=None, alpha=1e-3, trials=3, max_iterations=10):
        ...
        lmbda = np.concatenate((params, np.log2(np.abs(params) / 100.)))
        t = 0
        prelbo = np.inf
    ...
    def update_hypers(self, lmbda):
        print("Updating hyper-parameters...")

        # sample from posterior
        y = np.random.randn(100, len(lmbda) // 2)
        z = batch_T(y, lmbda)
            ...
            for n_t, (t_eval, S_batch, M_batch, inputs) in self.data_dict.items():
                ...
                for batch_inds in np.array_split(np.arange(n_samples), np.ceil(n_samples / self.n_devices)):
                    ...
                    for output, y_measured in zip(Y_pred, Y_meas):
                        ...
                        yCOV += np.sum(Y_error ** 2, 0) / z.shape[0]

        # update alpha
        params = np.concatenate([p.ravel() for p in self.params])

        # single alpha value (isotropic Gaussian prior)
        # self.alpha = len(params) / (np.sum(self.param_res(params) ** 2) + np.trace(self.Ainv) + 2. * self.a)
        # self.Alpha = self.alpha * np.ones_like(params)

        # independent priors for each parameter
        self.Alpha = 1. / (self.param_res(params) ** 2 + self.posterior_stdv ** 2 + 2. * self.a)

        # divide by number of observations
        yCOV = np.array(yCOV + self.b) / self.N

        # assume species have same measurement noise variance
        # yCOV[:self.n_s] = np.mean(yCOV[:self.n_s])

        # # assume metabolites have same measurement noise variance
        # if self.n_obs > self.n_s:
        #     yCOV = yCOV.at[self.n_s:].set(np.mean(yCOV[self.n_s:]))
    ...
    def elbo(self, lmbda, n_sample=10):

        # sample from posterior
        y = np.random.randn(n_sample, len(lmbda) // 2)
        z = batch_T(y, lmbda)
        ...
        for params in z:
            ...
            # compute negative log posterior (NLP)
            self.ELBO += np.sum(self.Alpha * self.param_res(params) ** 2) / 2. / n_sample
            ...
            for n_t, (t_eval, S_batch, M_batch, inputs) in self.data_dict.items():
                ...
                for batch_inds in np.array_split(np.arange(n_samples), np.ceil(n_samples / self.n_devices)):
                    ...
                    for output, y_measured in zip(Y_pred, Y_meas):
                        ...
                        # Determine SSE and gradient of SSE
                        self.ELBO += np.einsum('tk,kl,tl->', Y_error, self.Beta, Y_error) / 2. / n_sample

        # return NLP
        return self.ELBO

    def jacobian_elbo(self, lmbda, n_sample=10):
        ...
        for params, yi in zip(z, y):

            # compute gradient of negative log posterior
            grad_ELBO += np.einsum('i,i,ij->j', self.Alpha, self.param_res(params), grad_T(yi, lmbda)) / n_sample
            ...
            for n_t, (t_eval, S_batch, M_batch, inputs) in self.data_dict.items():
                ...
                for batch_inds in np.array_split(np.arange(n_samples), np.ceil(n_samples / self.n_devices)):
                    ...
                    for output, Z, Y_measured, input in zip(np.array(out_b),
                        ...
                        # sum over time and outputs to get gradient w.r.t params
                        grad_ELBO += np.einsum('i,ij->j', eval_grad_NLP(Y_error, self.Beta, G), grad_T(yi, lmbda)) / n_sample
